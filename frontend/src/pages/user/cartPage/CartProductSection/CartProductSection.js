...
function CartProduct({
  ...
  const checkQuantity = () => {
    let stockQuantity = product.informationProduct.productQuantities.find(
      (quantity) => quantity.sizeID === product.sizeID
    ).quantity;
    ...
    <div className="card-product d-flex">
      <div className="image-product">
        <Link to={"/product?productID=" + product.productID}>
          <img
            src={product.informationProduct.productImages[0].imagePath}
            alt={""}
          />
        </Link>
      </div>
      <div className="product__info">
          ...
          <div className="wrap-product-detail-properties d-flex ">
            {product.informationProduct.productSizes ? (
              product.informationProduct.productSizes.map((size, index) =>
                product.informationProduct.productQuantities.find(
                  (quantity) => quantity.sizeID === size.sizeID
                ) ? (
                  product.informationProduct.productQuantities.find(
                    (quantity) => quantity.sizeID === size.sizeID
                  ).quantity === 0 ? (
                    ...
                    <div
                      key={index}
                      className={`size-wrap size ${
                        product.sizeID === size.sizeID ? "selected-size" : ""
                      }`}
        ...
        <div className="product__price d-flex align-items-center">
          <div className="product__price__sale">
            {formatter(
              product.informationProduct.productPrice * product.quantityPurchase
            )}
          </div>
        </div>
        <div className="product__quantity d-flex">
          <button
            type="button"
            className="btn btn-light product__quantity__icon d-flex align-items-center justify-content-center"
            onClick={() => {
              if (product.quantityPurchase > 1) {
                handleDecreaseAmount(1);
              }
            }}
          >
            -
